...
let DashboardView: React.ComponentType<any> | null = null;
let OrderFormView: React.ComponentType<any> | null = null;
let OrderListView: React.ComponentType<any> | null = null;
let PieceOrderFormView: React.ComponentType<any> | null = null;
let SettingsView: React.ComponentType<any> | null = null;
let WeightCalculatorView: React.ComponentType<any> | null = null;
let EngineeringRegistryView: React.ComponentType<any> | null = null;
let HistoryView: React.ComponentType<any> | null = null;

try {
  DashboardView = require('./components/DashboardView').default;
} catch (e) {
  console.warn('DashboardView não encontrado', e);
}

try {
  OrderFormView = require('./components/OrderFormView').default;
} catch (e) {
  console.warn('OrderFormView não encontrado', e);
}

try {
  OrderListView = require('./components/OrderListView').default;
} catch (e) {
  console.warn('OrderListView não encontrado', e);
}

try {
  PieceOrderFormView = require('./components/PieceOrderFormView').default;
} catch (e) {
  console.warn('PieceOrderFormView não encontrado', e);
}

try {
  SettingsView = require('./components/SettingsView').default;
} catch (e) {
  console.warn('SettingsView não encontrado', e);
}

try {
  WeightCalculatorView = require('./components/WeightCalculatorView').default;
} catch (e) {
  console.warn('WeightCalculatorView não encontrado', e);
}

try {
  EngineeringRegistryView = require('./components/EngineeringRegistryView').default;
} catch (e) {
  console.warn('EngineeringRegistryView não encontrado', e);
}

try {
  HistoryView = require('./components/HistoryView').default;
} catch (e) {
  console.warn('HistoryView não encontrado', e);
}

// Função segura de storage com fallback
const safeStorage = {
  getItem: (key: string) => {
    try {
  ...
  if (hasError) {
    return (
      <div className="min-h-screen bg-[#001a35] flex items-center justify-center p-6">
        <div className="max-w-md bg-white rounded-[3rem] p-10 text-center shadow-2xl">
          <div className="w-16 h-16 bg-red-100 rounded-2xl flex items-center justify-center mx-auto mb-6">
            <span className="text-4xl">⚠️</span>
          </div>
          <h1 className="text-2xl font-black text-[#002855] mb-4 uppercase">Erro de Carregamento</h1>
          <p className="text-slate-600 mb-6">Ocorreu um erro ao carregar a aplicação. Tente recarregar a página.</p>
          <button 
            onClick={() => window.location.reload()}
            className="w-full py-3 bg-[#002855] text-white font-black rounded-xl hover:bg-[#001328] transition-all"
          >
            Recarregar Página
          </button>
        </div>
...
const NavItem: React.FC<NavItemProps> = ({ icon, label, active, collapsed, onClick }) => (
  <button 
    ...
    className={`w-full flex items-center gap-3 p-3.5 rounded-xl transition-all duration-300 ${
      active 
        ? 'bg-[#FFB800] text-[#001a35] font-black shadow-lg shadow-[#FFB800]/20' 
        : 'text-slate-400 hover:bg-white/5 hover:text-white'
    } ${collapsed ? 'justify-center' : ''}`}
  >
    <span className={`w-6 h-6 flex-shrink-0 ${active ? 'text-[#001a35]' : ''}`}>
      {icon}
    </span>
...
const ComponentPlaceholder: React.FC<{ name: string }> = ({ name }) => (
  <div className="flex flex-col items-center justify-center h-96 bg-white rounded-2xl border-2 border-dashed border-slate-200">
    ...
    <h3 className="text-xl font-black text-slate-600 mb-2">{name}</h3>
    <p className="text-slate-400 text-sm">Este componente ainda não foi carregado corretamente.</p>
  </div>
);
...
const App: React.FC = () => {
  ...
  // Carregamento inicial com fallback seguro
  useEffect(() => {
    try {
      const savedOrders = safeStorage.getItem('prod_orders');
      const savedUsers = safeStorage.getItem('sa_users');
      const savedSectors = safeStorage.getItem('sa_sectors');
      const savedSubSectors = safeStorage.getItem('sa_subsectors');
      const savedSheets = safeStorage.getItem('sa_sheets');
      const savedTubesRound = safeStorage.getItem('sa_tubes_round');
      const savedTubesSquare = safeStorage.getItem('sa_tubes_square');
      const savedTubesRect = safeStorage.getItem('sa_tubes_rect');
      const savedConfig = safeStorage.getItem('sa_config_v2');
      const savedHistory = safeStorage.getItem('sa_load_history');
      const savedLibrary = safeStorage.getItem('sa_eng_library');
  ...
  // Salvamento automático com fallback seguro
  useEffect(() => { safeStorage.setItem('prod_orders', JSON.stringify(orders)); }, [orders]);
  useEffect(() => { safeStorage.setItem('sa_load_history', JSON.stringify(loadHistory)); }, [loadHistory]);
  useEffect(() => { safeStorage.setItem('sa_eng_library', JSON.stringify(library)); }, [library]);
  useEffect(() => { safeStorage.setItem('sa_users', JSON.stringify(users)); }, [users]);
  useEffect(() => { safeStorage.setItem('sa_sectors', JSON.stringify(sectors)); }, [sectors]);
  useEffect(() => { safeStorage.setItem('sa_subsectors', JSON.stringify(subSectors)); }, [subSectors]);
  useEffect(() => { safeStorage.setItem('sa_sheets', JSON.stringify(sheets)); }, [sheets]);
  useEffect(() => { safeStorage.setItem('sa_tubes_round', JSON.stringify(tubesRound)); }, [tubesRound]);
  useEffect(() => { safeStorage.setItem('sa_tubes_square', JSON.stringify(tubesSquare)); }, [tubesSquare]);
  useEffect(() => { safeStorage.setItem('sa_tubes_rect', JSON.stringify(tubesRect)); }, [tubesRect]);
  useEffect(() => { safeStorage.setItem('sa_config_v2', JSON.stringify(config)); }, [config]);
  ...
  const runAiAnalysis = async () => {
    setIsAnalyzing(true);
    try {
      // Placeholder para análise IA - implementar conforme necessário
      const insight = "Sistema de análise IA configurado. Pressione novamente para atualizar insights.";
            ...
            <form onSubmit={handleSetup} className="space-y-6">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  placeholder="EX: ENCARREGADO GERAL" 
                  className="w-full px-6 py-5 rounded-2xl bg-slate-50 border-2 border-slate-100 font-black text-[#002855] outline-none focus:border-[#FFB800] uppercase" 
                />
              </div>
              <div className="space-y-2">
              ...
              <button 
                type="submit" 
                className="w-full py-6 bg-[#002855] text-[#FFB800] font-black uppercase text-xs rounded-2xl shadow-xl flex items-center justify-center gap-3 hover:bg-[#001328] transition-all active:scale-95 border-b-4 border-[#FFB800]"
              >
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <button 
                    ...
                    className="absolute right-6 top-1/2 -translate-y-1/2 text-slate-300 hover:text-[#002855]"
                  >
      ...
      <div className="min-h-screen flex bg-slate-50 overflow-hidden">
        ...
        <aside className={`bg-[#001a35] text-white transition-all duration-300 ${isSidebarOpen ? 'w-64' : 'w-20'} flex flex-col shadow-2xl z-20`}>
          ...
          <nav className="flex-1 mt-6 px-4 space-y-2 overflow-y-auto custom-scrollbar">
            <NavItem 
              ...
              onClick={() => { setReplicateOrderData(null); setActiveTab('dashboard'); }} 
            />
            <NavItem 
              icon={<ListTodo />} 
              ...
              collapsed={!isSidebarOpen} 
              onClick={() => { setReplicateOrderData(null); setActiveTab('orders'); }} 
            />
            <div className="py-2"><div className={`h-px bg-white/10 mb-2 ${!isSidebarOpen && 'mx-4'}`}></div></div>
            <NavItem 
              icon={<PlusCircle />} 
              label="Nova OP Simples" 
              active={activeTab === 'new-order'} 
              collapsed={!isSidebarOpen} 
              onClick={() => { setReplicateOrderData(null); setActiveTab('new-order'); }} 
            />
            <NavItem 
              icon={<TableIcon />} 
              label="Detalhamento" 
              active={activeTab === 'piece-order'} 
              collapsed={!isSidebarOpen} 
              onClick={() => { setReplicateOrderData(null); setActiveTab('piece-order'); }} 
            />
            <NavItem 
              icon={<Scale />} 
              label="Cálculo de Peso" 
              active={activeTab === 'weight-calc'} 
              collapsed={!isSidebarOpen} 
              onClick={() => setActiveTab('weight-calc')} 
            />
            <NavItem 
              icon={<Compass />} 
              label="Cadastro ENG" 
              active={activeTab === 'eng-registry'} 
              collapsed={!isSidebarOpen} 
              onClick={() => setActiveTab('eng-registry')} 
            />
            <NavItem 
              icon={<HistoryIcon />} 
              label="Histórico Carga" 
          ...
          <div className="p-4 border-t border-white/5 bg-[#001328] space-y-2">
            <button 
              ...
              className={`w-full flex items-center gap-3 p-3 rounded-xl bg-[#FFB800]/10 hover:bg-[#FFB800]/20 transition-all text-[#FFB800] font-bold border border-[#FFB800]/20 ${!isSidebarOpen && 'justify-center'}`}
            >
            ...
            <button 
              ...
              className={`w-full flex items-center gap-3 p-3 rounded-xl bg-rose-500/10 hover:bg-rose-500 text-rose-500 hover:text-white transition-all font-bold border border-rose-500/20 ${!isSidebarOpen && 'justify-center'}`}
            >
        ...
        <main className="flex-1 flex flex-col h-screen overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-8 bg-[#f8fafc]">
            ...
            {activeTab === 'dashboard' && (
              DashboardView ? (
                <DashboardView orders={orders} sectors={sectors} subSectors={subSectors} users={users} />
              ) : (
                <ComponentPlaceholder name="Dashboard" />
              )
            )}
            ...
            {activeTab === 'orders' && (
              OrderListView ? (
                ...
                <ComponentPlaceholder name="Ordens de Serviço" />
              )
            )}
            ...
            {activeTab === 'new-order' && (
              OrderFormView ? (
                <OrderFormView 
                  initialData={replicateOrderData} 
                  sectors={sectors} 
                  subSectors={subSectors} 
                  onSubmit={addOrder} 
                  onCancel={() => { setReplicateOrderData(null); setActiveTab('dashboard'); }} 
                />
              ) : (
                <ComponentPlaceholder name="Nova OP Simples" />
              )
            )}

            {activeTab === 'piece-order' && (
              PieceOrderFormView ? (
                <PieceOrderFormView 
                  initialData={replicateOrderData} 
                  sectors={sectors} 
                ...
                <ComponentPlaceholder name="Detalhamento" />
              )
            )}

            {activeTab === 'weight-calc' && (
              WeightCalculatorView ? (
                ...
                <ComponentPlaceholder name="Cálculo de Peso" />
              )
            )}
            ...
            {activeTab === 'eng-registry' && (
              EngineeringRegistryView ? (
                ...
                <ComponentPlaceholder name="Cadastro ENG" />
              )
            )}
            ...
            {activeTab === 'history' && (
              HistoryView ? (
                ...
                <ComponentPlaceholder name="Histórico Carga" />
              )
            )}
              ...
              isSettingsUnlocked ? (
                SettingsView ? (
                  <SettingsView 
                    ...
                    setSheets={setSheets} 
                    tubesRound={tubesRound} 
                    setTubesRound={setTubesRound} 
                    tubesSquare={tubesSquare} 
                    setTubesSquare={setTubesSquare} 
                    tubesRect={tubesRect} 
                    setTubesRect={setTubesRect} 
                  />
                ) : (
                  <ComponentPlaceholder name="Configurações" />
                )
              ) : (
                <div className="max-w-md mx-auto mt-20 p-10 bg-white rounded-[3rem] shadow-2xl border-2 border-slate-100 animate-in zoom-in-95 duration-500 flex flex-col items-center">
                  ...
                  <form onSubmit={handleSettingsUnlock} className="w-full space-y-4">
                    ...
                    <button 
                      type="submit" 
                      className="w-full py-5 bg-[#002855] text-[#FFB800] font-black uppercase text-[10px] rounded-2xl shadow-xl flex items-center justify-center gap-3 hover:bg-[#001a35] transition-all"
                    >
